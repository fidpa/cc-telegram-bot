...
export async function checkPendingAskUserRequests(
  ...
  for await (const filename of glob.scan({ cwd: askUserDir, absolute: false })) {
    const filepath = `${askUserDir}/${filename}`;
    try {
...
function formatWithinLimit(
  ...
  // Redact secrets BEFORE formatting (SEC-008: output redaction)
  const safe = redactSecrets(content);

  let display =
    safe.length > safeLimit ? safe.slice(0, safeLimit) + "..." : safe;
  ...
  if (formatted.length > TELEGRAM_MESSAGE_LIMIT) {
    ...
    display = safe.slice(0, Math.floor(safeLimit * ratio * 0.95)) + "...";
  ...
  return async (statusType: string, content: string, segmentId?: number) => {
    try {
      if (statusType === "thinking") {
        ...
        // Redact secrets from thinking output (SEC-008)
        const safe = redactSecrets(content);
        const preview =
          safe.length > 500 ? safe.slice(0, 500) + "..." : safe;
        ...
        // Redact secrets from tool status (SEC-008)
        const toolMsg = await ctx.reply(redactSecrets(content), { parse_mode: "HTML" });
