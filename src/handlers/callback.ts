...
export async function handleCallback(ctx: Context): Promise<void> {
  ...
  // 3. Load request file from secured directory
  const homeDir = process.env.HOME || "/tmp";
  const requestFile = `${homeDir}/.cc-telegram-bot/ask-user/ask-user-${requestId}.json`;
  let requestData: {
    question: string;
    options: string[];
    status: string;
  ...
  // 4. Validate user ownership (H3: prevent cross-user hijacking)
  if (!requestData.user_id || String(requestData.user_id) !== String(userId)) {
...
async function handleResumeCallback(
  ...
  if (session.isActive) {
    await ctx.answerCallbackQuery({ text: "Sitzung bereits aktiv" });
    return;
  }

  // Resume the selected session
  const [success, message] = session.resumeSession(sessionId, userId!);
  ...
  await ctx.answerCallbackQuery({ text: "Sitzung fortgesetzt!" });
